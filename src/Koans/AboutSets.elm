...
testSuite =
    suite "About Sets"
        [ test "a set can be created from a list"
            (assertEqualSets (Set.fromList [ 1, 2, 3 ]) (Set.insert 3 <| Set.insert 2 <| Set.insert 1 <| Set.empty))
        , test "a set cannot contain duplicates"
            (assertEqualSets (Set.fromList [ 1, 2, 3, 3, 2, 1 ]) (Set.fromList [ 1, 2, 3 ]))
        , test "sets may be empty"
            (assertEqualSets Set.empty (Set.fromList []))
        , test "or contain a single value"
            (assertEqualSets (Set.singleton 0) (Set.fromList [ 0 ]))
